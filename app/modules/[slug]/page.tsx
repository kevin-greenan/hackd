  ...
  return (
    <div className="mt-4 rounded-md border border-border bg-white p-3">
      ...
      {instance?.statusMessage ? (
        <p className="mt-2 rounded-md bg-surface px-3 py-2 text-sm text-muted-foreground">
          {instance.statusMessage}
        </p>
      ) : null}
...
export default async function ModuleDetailPage({
  ...
  return (
    <AppShell user={user} area="learner">
      <div className="mb-6">
        <Link className="text-sm font-semibold text-primary hover:opacity-80" href="/dashboard">
          Back to dashboard
        </Link>
      </div>
            ...
            {learningModule.challenges.length > 0 ? (
              <div className="mt-4 grid gap-3">
                {learningModule.challenges.map((challenge) => (
                  <div key={challenge.id} className="rounded-md border border-border bg-surface p-4">
